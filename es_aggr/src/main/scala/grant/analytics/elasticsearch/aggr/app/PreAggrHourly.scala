...
import grant.analytics.common.event.alternative.{UserEngageEvent, AnalyticsEvent, UserViewEvent, DefaultUserEventParserContainer}
...
object PreAggrHourly extends App with ClasspathURLEnabler{
  ...
  def getGeneralEvents[T<:AnalyticsEvent](event:T):List[GeneralEvent] = {
    ...
    event.event_type match {
      case "user.view" => {
        val view = event.asInstanceOf[UserViewEvent]
        List(GeneralEvent(event.event_type, event.page_uuid.toString, minute, event.device, event.referral, event.unique.toString, 1,  if(view.viewcounts == 1) 1 else 0, if(view.recirculation) 1 else 0))
      }
      case "user.engage" => {
        val engage = event.asInstanceOf[UserEngageEvent]
        List(GeneralEvent(event.event_type, event.page_uuid.toString, minute, event.device, event.referral, event.unique.toString, 1, 0, 0, engage.engage_time, engage.comment_attention))
      }
      case _ => {
        List(GeneralEvent(event.event_type, event.page_uuid.toString, minute, event.device, event.referral, event.unique.toString))
      }
    }
  }
  ...
  def generateEntity1(uuid:String, event_type:String, minute:Int, counts:Long, viewCounts:Long, recirculation:Long, attention:Long, comment_attention:Long, device:String, referral:String):List[UUID_Feature_Minute_Counts_Device_Referral] = {
    ...
    event_type match {
      case "user.view" => {
        List(
          UUID_Feature_Minute_Counts_Device_Referral(uuid, "feature1", minute, recirculation, device, referral),
          UUID_Feature_Minute_Counts_Device_Referral(uuid, "feature2", minute, viewCounts, device, referral),
          UUID_Feature_Minute_Counts_Device_Referral(uuid, "feature3", minute, counts, device, referral)
        )
      }
      case "analytics.engage" => {
        List(
          UUID_Feature_Minute_Counts_Device_Referral(uuid, "feature4", minute, comment_attention, device, referral),
          UUID_Feature_Minute_Counts_Device_Referral(uuid, "feature5", minute, attention, device, referral)
        )
      }
    }
  }
  ...
  def run(sqlContext:SQLContext, config: PreAggrConfiguration):Unit = {
    ...
    getTimeRange(config).foreach(hour_ms => {
      ...
      val event_parser_container = new DefaultUserEventParserContainer
      ...
      EsSpark.saveJsonToEs(
        ...
        event_ds.filter(event => {
          event.event_type.equals("user.view") ||
          ...
          val features = event.event_type match {
            case "user.view" => List("new_views", "views")
